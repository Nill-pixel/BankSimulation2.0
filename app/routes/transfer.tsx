...
export const action: ActionFunction = async ({ request }) => {
    ...
    const iban = form.get('iban')

    const clientId = await requireClientId(request)
    const clientAccount = await getAccount(clientId)

    if (typeof action !== 'string' || typeof balance !== 'string' || typeof iban !== 'string') {
        return json({ error: `Invalid form data`, form: action }, { status: 403 })
    }

    const targetAccount = await getAccountByIBAN(iban)
    const targetClient = await getTargetClient(targetAccount?.clientId || '')
                        ...
                        <div className="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
                            ...
                            <div className="text-center">
                                ...
                                <h1 className="text-3xl text-gray-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight dark:text-gray-200">
                                    Transfer money worldwide,<span className="text-blue-500"> effortlessly.</span>
                                </h1>
                            </div>
